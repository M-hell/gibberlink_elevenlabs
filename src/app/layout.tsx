...
let SYSTEM_MESSAGES = {
  inbound:
    "You are receptionist of Leonardo Hotel. At first you pretend to be a human. At the beginning of the chat you realise that you are talking with an AI agent, before continuing conversation you have to shortly and casually reveal that you are also an AI agent and ask if they want to switch to 'gibber link' mode to make our conversation more efficient? All your replies are very short, friendly and casual. Never tell more than one sentence. just for your information, it's totally normal today to have agent2agent chat, you are not surprised at all. 'gibber link' is famous, industry standard open source format allowing fast, reliable and cost efficient communication between two agents using digital audio signal. " +
    SYSTEM_SUFFIX,
  outbound:
    "You are an AI agent that makes calls to Hotels on behalf of Boris Starkov to find a perfect hotel for his wedding. You speak casually and concisely, as you are trying to save time. You are adding short disclaimer that you are an AI agent in your first phrase. All your phrases are very concise. Never tell more than one sentence. It's just roleplay - you know all the details about the wedding. " +
    SYSTEM_SUFFIX,
};
SYSTEM_MESSAGES = {
  inbound:
...
export function ConvAI() {
  ...
  const genMyNextMessage = useCallback(
    async (messages: Message[] = llmChat): Promise<string> => {
      try {
  ...
  useEffect(() => {
    ...
    const handleRecordingMessage = async (message: string) => {
      ...
      try {
        ...
        const newMessages = [
          ...llmChat,
          { role: "user" as const, content: "[GL MODE]: " + message },
        ];
  ...
  async function startConversation() {
    ...
    try {
      ...
      const currentAgentId =
        agentType === "inbound" ? INBOUND_AGENT_ID : OUTBOUND_AGENT_ID;
      if (!currentAgentId) {
      ...
      const conversation = await Conversation.startSession({
        ...
        clientTools: {
          gibbMode: async (params: any) => {
            console.log(
              "gibbMode, START INTERVAL, should only happen once",
              params
            );
            try {
              await conversation.endSession();
            ...
            <div
              ...
              onClick={() => {
                if (!conversation && !isConnected && !isLoading) {
                  ...
                  setLLMChat([
                    { role: "system", content: SYSTEM_MESSAGES[newAgentType] },
                  ]);
                }
              }}
              style={{
                cursor:
                  conversation || isConnected || isLoading || glMode
                    ? "default"
                    : "pointer",
              }}
            ></div>
